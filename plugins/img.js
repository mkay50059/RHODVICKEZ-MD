...
cmd({
    ...
    try {
        ...
        if (!query) {
            return reply("*Please give Rhodvick a search query.*");
        }

        
        await reply(`*🔎 Rhodvick isDownloading Images For ${query}...*`);
        ...
        if (!response.data || !response.data.result || response.data.result.length === 0) {
            return reply("*No results found. Please give Rhodvick another keyword.*");
        }
